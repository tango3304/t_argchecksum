...
class MyCheckSum:
	try:
		...
		def common_checksum(function):
			def values(*args):
				# {ip_checksum}Function Get ReturnValue
				# Calculate One's Complement
				# XOR Calculation
				# {ip_checksum}関数のReturn値を取得
				# 1の補数を計算
				# XOR計算
		...
		def iptcp_header_module(self):
			...
			# byte(Hexadecimal) => str(BinaryNumber)
			# バイト型(16進数) => 文字型(2進数)
			binary_numbers = format(int.from_bytes(self.args[0], 'big'), '0160b')

			# Get Binary Length
			# Obtain values in increments of 16 and convert the base to binary and add them
			# バイナリの長さを取得
			# 16刻みで値を取得し基数を２進数にし足していく
